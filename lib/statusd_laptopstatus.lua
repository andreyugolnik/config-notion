...
if not statusd_laptopstatus then
  statusd_laptopstatus = {
    ...
    ac_state = {
        "/sys/class/power_supply/AC/state",
...
local function get_thermal()
    ...
    if pcall(function ()
        ...
        for _, file in pairs(files) do
            ...
            local t = tonumber(file:read("*all"))
            temp = temp + t / 1000
            file:close();
        end
        temp = temp / #files
        ...
        --status, _, temp = string.find(file:read("*all"), "temperature:%s+(%d+).*")
    ...
    then if temp >= statusd_laptopstatus.temperature_important then
        ...
        if temp >= statusd_laptopstatus.temperature_critical then
            hint = "critical" end
            return {temperature=string.format("%02d°C", temp), hint=hint}
